...
import java.io.*;
public class MinesweeperWithAI extends JPanel implements ActionListener
{
    private int width;
    private int height;
    ...
    public MinesweeperWithAI(int cellW, int cellH)
    {
        ...
        numBombs = 99;
        MinesweeperAIv1.setPIVs(numBombs,numCellsX,numCellsY);
        //MinesweeperAIv2.setPIVs(numBombs,numCellsX,numCellsY);
    ...
    {
        System.out.println("hello "+firstClick);        
        ...
        // [row,column,reveal(0)/flag(1)]

        //I never finished AIv2 so it still runs v1
        int[] move = MinesweeperAIv1.makeMove(visableBoard);
        //int[] move = MinesweeperAIv2.makeMove(visableBoard);
        ...
        if(!firstClick)
        {
            ...
            System.out.println(move[0]*numCellsX+move[1]);
            System.out.println(bombArr[move[0]*numCellsX+move[1]]);

            while(bombArr[move[0]*numCellsX+move[1]]==1)
            {
                System.out.println("regen");
                int rand = (int)(Math.random()*bombArr.length);
                bombArr[move[0]*numCellsX+move[1]] = bombArr[rand];
                bombArr[rand] = 1;
        ...
        if(move[2]==0)
        {
            if(visableBoard[move[0]][move[1]]==-3)
            {
                numFlagsPlaced--;
            }
            visableBoard[move[0]][move[1]] = board[move[0]][move[1]];
            if(visableBoard[move[0]][move[1]]==0)
            {
                ...
                checkZeroTiles(move[0],move[1],tempBoard);
            }
        }
        else if(move[2]==1)
        {
            ...
            if(visableBoard[move[0]][move[1]] == -3)
            {
                visableBoard[move[0]][move[1]] = -2;
                numFlagsPlaced--;
            }
            else
            {
                visableBoard[move[0]][move[1]] = -3;
                numFlagsPlaced++;
            }
        }
        checkGameOver();
